...
Règles :
- Par défaut, on **ignore** totalement tout (pos, lemma) déjà présent dans `data.json`
  (peu importe la valeur de `nooj`).
- Avec l’option `--legacy-nooj-only`, on revient au comportement historique :
  on ignore uniquement les entrées ayant un champ `nooj` non vide.
...
def render_report_html(
    ...
    if ignore_all_existing:
        note_text = (
            "Par défaut, les lemmes déjà présents dans <code>data.json</code> "
            "(quel que soit leur champ <code>nooj</code>) sont ignorés."
        )
    else:
        note_text = (
            "Mode <code>--legacy-nooj-only</code> activé : seuls les lemmes ayant un "
            "champ <code>nooj</code> non vide sont ignorés."
        )
...
<body>
  <div class="container">
    ...
    <div class="summary">
      ...
      <div class="footer-note">{note_text}</div>
    </div>
...
def process_phrases_ordered(
    ...
    Paramètres :
      - ignore_all_existing :
          * True  (défaut) : ignorer toute entrée déjà présente dans data.json
            pour ce (pos, lemma), peu importe 'nooj'.
          * False : n'ignorer que les entrées ayant un champ 'nooj' non vide
    ...
    for phrase, meta in phrases_dict.items():
        ...
        for span in soup.find_all("span", class_="ukr"):
            ...
            # --- Ignorer les entrées déjà présentes dans data.json ---
            existing = data[pos].get(lemma)
            if existing and isinstance(existing, dict):
                if ignore_all_existing:
                    # strict : toute entrée déjà présente est ignorée
                    processed.add(key)
                    continue
                else:
                    # mode legacy : on ignore seulement si 'nooj' est non vide
                    if existing.get("nooj"):
                        processed.add(key)
                        continue
            # ----------------------------------------------------------
...
def main():
    ...
    ap.add_argument(
        "--legacy-nooj-only",
        action="store_true",
        help="Revenir au comportement historique : n'ignorer que les entrées ayant un champ 'nooj' non vide."
    )
    ...
    # Par défaut : ignore_all_existing = True
    ignore_all_existing = not args.legacy_nooj_only

    entries_ordered, cards_ordered, errors = process_phrases_ordered(
        phrases_dict=phrases_dict,
        data_path=args.data,
        limit=args.limit,
        ignore_all_existing=ignore_all_existing,
    )
    ...
    html = render_report_html("".join(cards_ordered), counts, errors, ignore_all_existing=ignore_all_existing)
    try:
