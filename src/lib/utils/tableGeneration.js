...
/**
 * Rend le contenu d'une cellule de tableau grammatical.
 * Convertit une entrée (paire ou tableau de paires) en texte accentué.
 * @param {Array} entry - Entrée de données (paire [text, pos] ou tableau de paires)
 * @returns {string} Texte accentué, variantes séparées par « / »
 */
export function renderCell(entry) {
...
/**
 * Génère un tableau HTML de déclinaison pour un nom.
 * @param {object} casData - Données de déclinaison { cas: { nombre: entry } }
 * @param {string} cas - Cas grammatical actif (pour mise en gras)
 * @param {string} gender - Genre/nombre actif (pour mise en gras)
 * @returns {string} HTML du tableau
 */
export function generateTableNoun(casData, cas, gender) {
...
/**
 * Génère un tableau HTML de déclinaison pour un adjectif (ou cardinal, possessif, pronom).
 * Dimensions : cas × genre. Le vocatif est omis.
 * @param {object} casData - Données { cas: { genre: entry } }
 * @param {string} cas - Cas grammatical actif
 * @param {string} gender - Genre actif
 * @returns {string} HTML du tableau
 */
export function generateTableAdj(casData, cas, gender) {
  if (!casData) return "<table></table>";
...
/**
 * Génère un tableau HTML de conjugaison pour un verbe.
 * Sections : infinitif, présent, futur, passé, impératif, impersonnel + couple aspectuel.
 * @param {object} verbDetails - Données du verbe (inf, conj, coupl)
 * @param {object} wd - wordData complet (pour résoudre le couple aspectuel)
 * @returns {string} HTML du tableau + note couple aspectuel
 */
export function generateTableVerb(verbDetails, wd) {
